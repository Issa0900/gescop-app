// forked from design-sync lib/source-kit.mjs - synth entry must re-export default exports by name
//
// This repo's product components are all `export default function <Name>()`.
// The upstream synth entry emits only `export * from <file>`, and a star export
// never carries a module's default - so every default-exported component was
// discovered, documented and bundled, yet absent from window.<GLOBAL>, and every
// preview rendered "Element type is invalid ... got: undefined". The fork adds an
// explicit `export { default as <Name> }` alongside each star export.
//
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // A star export drops the module's default, so `export default function Foo()`
    // never reaches window.<GLOBAL>. Re-export each default under its declared
    // name. First declaration wins: two files defaulting the same name would be a
    // duplicate-export syntax error, and an explicit export already shadows any
    // same-named star export (no conflict with the `export *` lines above).
    ...
    for (const p of comps) {
      ...
      const sf = defProject.addSourceFileAtPathIfExists(p);
      const decls = sf?.getExportedDeclarations().get('default');
      if (!decls) continue;
      const real = decls.map((d) => d.getName?.()).find((n) => n && n !== 'default');
      if (!real || !/^[A-Z][A-Za-z0-9]*$/.test(real) || claimed.has(real)) continue;
      claimed.add(real);
      lines.push(`export { default as ${real} } from ${JSON.stringify(p)};`);
    }
    console.error(`  [FORK] re-exported ${claimed.size} default export(s) by name`);
