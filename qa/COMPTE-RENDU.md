...
- **Refonte UI/UX SaaS des 3 rapports GESCOP (Résolu - commit abe5f9e)** : transformation complète des 3 rapports en véritable produit SaaS moderne : Quotidien (SURVEILLER), Hebdomadaire (COMPRENDRE), Mensuel (PILOTER « Rapport Vitrine » 12 sections), sélecteur de rapport et visualiseur dynamique.
- **Charte rédactionnelle GESCOP pour toute l'application (Résolu)** : création du guide `docs/STYLE-REDACTIONNEL.md` (ton contrôleur de gestion senior, sobre, factuel, distinction `[FAIT]` / `[CALCUL]`, boutons à verbe d'action unique, vocabulaire financier rigoureux) et application à l'ensemble des pages de l'application.
- **Cartes KPI professionnelles et suppression des tirets (Résolu)** : élimination de tous les tirets bruts (`"-"`, `"——"`) au profit d'un statut explicite « Non mesuré », flèche de tendance `ArrowRight`, pills colorées et masquage des unités lorsque la donnée est absente.
