...
- **Refonte UI/UX SaaS des 3 rapports GESCOP (Résolu)** : transformation complète de la présentation des 3 rapports en véritable produit SaaS moderne à forte valeur décisionnelle :
  1. *Cartes de sélection (`ReportTypeCard.jsx`)* : hiérarchie décisionnelle explicite (Quotidien = SURVEILLER, Hebdomadaire = COMPRENDRE, Mensuel = PILOTER avec badge « Rapport Vitrine ») avec fonctionnalités clés et objectifs.
  2. *Rapport Quotidien (`DailyReportView.jsx`)* : scan exécutif en < 2 minutes (État général, 4 grosses cartes KPI CA/Marge/Trésorerie/Stock avec deltas et statuts, Points d'attention 01/02, Actions prioritaires du jour, Jauge de fiabilité unifiée).
  3. *Rapport Hebdomadaire (`WeeklyReportView.jsx`)* : 5 zones structurées (Synthèse, 4 KPI, Évolution S35-S39 à barres visuelles, Bilan croisé 2 colonnes « Ce qui s'améliore » vs « Ce qui se dégrade », Diagnostic rigoureux `[FAIT]` / `[CALCUL]` / `[FACTEUR]`, Priorités ① ② ③).
  4. *Rapport Mensuel « Rapport Vitrine » (`MonthlyReportView.jsx`)* : dossier exécutif complet de pilotage stratégique en 12 sections/pages (Couverture prestige, Executive Brief 4 piliers, Vue 4 quadrants 360°, Performance financière en cascade Waterfall avec analyse causale Volume/Prix/Mix/Coûts, Trésorerie & projections 30j/60j avec runway, Ventes & concentration Pareto, Stocks & alertes 🔴🟠🟡, Analyse transversale systémique signature GESCOP, Intelligence d'affaires qualifiée %, Simulateur interactif de scénarios « Et si... », Plan d'action tabulaire, Audit & fiabilité des sources).
  5. *Visualiseur universel (`ReportViewer.jsx`)* : routage intelligent vers chaque vue spécialisée, extraction et normalisation des données réelles et contextuelles (`reportDataExtractor.js`), et sélecteur de mode « Vue SaaS » vs « Texte Brut Markdown ».
