...
- **Constaté, non traité (hors périmètre) :** le pipeline Observation ne connaît que 14 concepts (`registry/conceptRegistry.ts`) et `semanticMatcher.ts` compare un nom de colonne sans espaces à des mots-clés qui en contiennent (« chiffre d affaires ») : presque aucune observation n'est produite (sur une ligne de commande typique, seule la date est reconnue). Le moteur KPI qui s'appuie sur `Observation` en reçoit donc très peu.
